...
class KinematicModel:
    ...
    def from_parameters(self, param_file):
        ...
        self.trackwidth = params["trackwidth"]
        self.wheelbase = params["wheelbase"]
        self.motion_ratio = params["motion_ratio"]
        self.steer_rate = params["steer_rate"]
        self.nonlin_motional_ratio = params["nonlin_motional_ratio"]
        ...
        self.bump_steer_back = params["bump_steer_back"]
        ...
        self.rear = self._generate_model(self.rear_shock_travel, self.steering_rack_delta, 
                                         self.rear_camber, self.bump_steer_back)

    def contact_patch(self, track_width, wheelbase, shock):
        x = track_width / 2
        y = wheelbase / 2
        z = shock / self.motion_ratio

        return np.array([x, y, z])

    
    def _generate_model(self, shock, steer, camber_gain, bump_steer):
        ...
        for i, shock in enumerate(shock_space):
            for j, steer in enumerate(steer_space):
                contact_patch_positions[i, j] = np.array([
                        self.trackwidth / 2, 
                        self.wheelbase / 2, 
                        i / self.motion_ratio if self.motion_ratio != 0 else 0])
                ...
                motion_ratio[i, j] = self.motion_ratio
                ...
                print(np.shape(relative_shock[0]))
